...
async def get_quotex_candles(email, password, asset, otp=None):
    try:
        ...
        try:
            ...
            # Set browser-like headers to avoid IP blocks
            try:
                ...
                pass  # headers not critical, continue anyway
            ...
            return None, "Timeout — Quotex server slow to respond, try again"
        finally:
            builtins.input = orig_input

        if not raw:
            return None, "No candles received from Quotex"
        ...
        if len(candles)<30:
            return None, f"Only {len(candles)} candles received"
        return candles, None
