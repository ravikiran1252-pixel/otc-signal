...
async def get_quotex_candles(email, password, asset, otp=None):
    try:
        ...
        try:
            ...
            ok, msg = await asyncio.wait_for(client.connect(), timeout=30)
            if not ok:
                s = str(msg).lower()
                if any(x in s for x in ['otp','code','verify','2fa','403','forbidden','eof']):
                    return None, "OTP_REQUIRED"
                return None, f"Login failed: {msg}"
            raw = await asyncio.wait_for(client.get_candles(asset, 60, 200), timeout=20)
            await client.close()
        except EOFError:
            return None, "OTP_REQUIRED"
        ...
        s=str(e).lower()
        if any(x in s for x in ['eof','otp','verify','2fa','403','forbidden']):
            return None, "OTP_REQUIRED"
        return None, str(e)
...
async def generate_signal(req: SignalRequest):
    ...
    if err == "OTP_REQUIRED" or (err and any(x in str(err).lower() for x in ['403','forbidden','otp','eof'])):
...
HTML = open("/app/index.html").read() if os.path.exists("/app/index.html") else """<!DOCTYPE html>
...
<div style="font-size:11px;color:#6b7ab5;margin-top:-4px">Connects directly to Quotex for real OTC prices</div></div>
...
<div style="font-size:13px;color:#6b7ab5;margin-bottom:14px;line-height:1.6">Quotex sent a code to your email or phone. Enter it below:</div>
...
<div style="font-size:11px;color:#374151;text-align:center;padding:6px 0 12px">&#9888;&#65039; Real Quotex OTC data &middot; 1 min expiry &middot; Trade responsibly</div>
</div>
</div>
<script>
...
var msgs=['Connecting to Quotex...','Authenticating...','Fetching real OTC candles...','Computing RSI, MACD, Bollinger...','Analysing Stochastic and EMA...','Scanning candle patterns...','Running backtest engine...','Finalising signal...'];
var mi=0,mTick=null;
