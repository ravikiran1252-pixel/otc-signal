...
async def get_quotex_candles(email, password, asset, otp=None):
    try:
        import websockets, json, time, urllib.parse

        # Decode cookie
        session_cookie = urllib.parse.unquote(password)
            ...
            try:
                async with websockets.connect(uri, extra_headers=headers, ping_interval=20) as ws:
                    ...
                    while True:
                        ...
                        # Connected - authenticate with session
                        if msg.startswith("0"):
                            await send("40")
                            continue
                        ...
                        if msg.startswith("40"):
                            auth = json.dumps(["authorization", {
                                "session": session_cookie,
                                "isDemo": 1
                            }])
                            await send(f"42{auth}")
                            continue
                        ...
                        if msg.startswith("42"):
                            try:
                                ...
                                # Auth success
                                if event == "authorization" and not candle_requested:
                                    if isinstance(payload, dict) and payload.get("token"):
                                        authenticated = True
                                        # Request candle history
                                        now = int(time.time())
                                        req = json.dumps(["history/load", {
                                            "asset": asset,
                                            "index": now,
                                            "time": 60,
                                            "offset": 200
                                        ...
                                        await send(f"42{req}")
                                        candle_requested = True

                                # Candle data received
                                elif event in ["history/load", "candles"] or "candle" in str(event).lower():
                                    raw = []
        ...
        if len(candles) < 30:
            ...
            raise Exception(f"WebSocket got only {len(candles)} candles, trying pyquotex")

        return candles, None

    except Exception as ws_err:
        # Fallback: try pyquotex with session cookie
        try:
            ...
            try:
                ...
                if not ok:
                    s = str(msg).lower()
                    if any(x in s for x in ['otp','code','verify','2fa','eof','unknown','email','change']):
                ...
                return None, "Timeout connecting to Quotex"
            finally:
            ...
            if len(candles2)<30:
                return None, f"Only {len(candles2)} candles"
            return candles2, None

        except Exception as e:
            s=str(e).lower()
            if any(x in s for x in ['eof','otp','verify','2fa','unknown','email']):
                return None, "OTP_REQUIRED"
            return None, f"Error: {str(e)} | WS error: {str(ws_err)}"
...
async def generate_signal(req: SignalRequest):
    ...
    if err == "OTP_REQUIRED" or (err and any(x in str(err).lower() for x in ['otp','eof','verify'])):
...
.card h2{font-size:12px;font-weight:700;color:#9b7fff;margin-bottom:12px;text-transform:uppercase;letter-spacing:1px}
...
textarea{width:100%;background:#1a1a2e;border:1px solid #7c3aed;border-radius:10px;padding:12px 14px;color:#e2e8ff;font-size:11px;margin-bottom:10px;font-family:monospace;outline:none;resize:none;height:80px}
...
.err{background:#450a0a;border:1px solid #ef444466;border-radius:12px;padding:14px;color:#ef4444;font-size:13px;margin-top:10px;display:none}
...
.otp-box{display:none;background:#111127;border:2px solid #7c3aed55;border-radius:16px;padding:20px;margin-top:12px}
...
<div class="card"><h2>&#128279; Quotex Login</h2>
<input id="email" type="email" placeholder="Quotex Email"/>
<div style="font-size:11px;color:#9b7fff;margin-bottom:6px;font-weight:700">Session Cookie (paste from Firefox):</div>
<textarea id="password" placeholder="Paste your Quotex session cookie here&#10;(From Firefox: javascript:alert(document.cookie))"></textarea>
<div style="font-size:11px;color:#6b7ab5">Paste the session cookie from Firefox for direct connection</div>
</div>
...
<div style="font-size:13px;color:#6b7ab5;margin-bottom:14px;line-height:1.6">Quotex sent a code to your email. Enter it below:</div>
...
<button onclick="submitOTP()" style="width:100%;padding:14px;border-radius:12px;background:#7c3aed;color:#fff;font-weight:800;font-size:15px;border:none;cursor:pointer;margin-top:4px">&#10003; Verify and Get Signal</button>
</div>
...
var msgs=['Connecting to Quotex WebSocket...','Authenticating with session...','Fetching real OTC candles...','Computing RSI, MACD, Bollinger...','Analysing Stochastic and EMA...','Scanning candle patterns...','Running backtest engine...','Finalising signal...'];
var mi=0,mTick=null;
...
if(!creds.password){showErr('Please paste your session cookie.');return;}
