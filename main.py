...
async def get_quotex_candles(email, password, asset, otp=None):
    ...
    session_cookie = password  # we now pass cookie as password field
    
    # If it looks like an actual password (short, no %), use old method
    # If it looks like a cookie (long encoded string), use cookie method
    use_cookie = len(password) > 50 and '%' in password

    if use_cookie:
        # ── USE SESSION COOKIE (most reliable) ──
        try:
            headers = {
            ...
            # Connect to Quotex WebSocket with cookie
            from pyquotex.stable_api import Quotex
            user_data_dir = f"/tmp/qx_{hashlib.md5(email.encode()).hexdigest()[:8]}"
            os.makedirs(user_data_dir, exist_ok=True)
            
            # Save session cookie to file so pyquotex uses it
            ...
            if not ok:
                s = str(msg).lower()
                if any(x in s for x in ['otp','code','verify','2fa','eof']):
            ...
            return None, f"Cookie login error: {str(e)}"
...
<div class="card"><h2>&#128279; Quotex Account</h2>
  ...
  <div style="font-size:11px;color:#6b7ab5;margin-top:-4px">Use session cookie in password field for best results</div>
</div>
...
<div class="otp-box" id="otpBox">
  ...
  <div style="font-size:13px;color:#6b7ab5;margin-bottom:13px;line-height:1.6">Quotex requires verification. Check your email for a code OR enter your Quotex email to confirm:</div>
